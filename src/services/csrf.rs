...
//!
//! This server-side check inspects `Origin` on every state-changing admin
//! request and rejects when the value isn't on our allowlist. A browser
//! always sets `Origin` on POST/PUT/DELETE from a real page; curl /
//! server-to-server calls don't, which is exactly what we want to reject on
//! admin write paths.
//!
