...
//! token we stored on `POST /register`). Plugin-side verification is
...
///
/// `role_link_token` is the raw API token RoleLogic gave us at
...
// token in [crate::services::session].
// -------------------------------------------------------------------------
...
mod tests {
    ...
    const PLUGIN_AUD: &str = "https://plugin.example/steam-player-role";
