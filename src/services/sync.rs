...
use crate::services::condition_eval::{evaluate_conditions, GameAchievementRow, UserCacheRow};
use crate::AppState;
...
pub async fn sync_for_player(discord_id: &str, state: &AppState) -> Result<(), AppError> {
    ...
    for (guild_id, role_id, api_token, conditions) in &role_links {
        let qualifies = evaluate_conditions(conditions, &user_cache, &game_achievements);
...
pub async fn sync_for_role_link(
    ...
    if needs_achievement_eval {
        ...
        for (discord_id, steam_id, owned_games, groups, steam_level, account_created, total_games_owned, is_vac_banned, is_game_banned, country_code) in candidates {
            ...
            if evaluate_conditions(&conditions, &uc, &achievements) {
        ...
        // Atomic replace
        rl_client.replace_users(guild_id, role_id, &qualifying_ids, &api_token).await?;
